...
class TestGenerator:
    ...
    def generate_tests(self, source_file: str, analysis: CodeAnalysis) -> int:
        ...
        try:
            ...
            self.logger.error(f"Erreur lors de la génération des tests pour {source_file}: {e}")
            return 0
    
    def _create_prompt(self, filename: str, source_code: str, analysis: CodeAnalysis) -> str:
        """
    ...
    def _post_process_tests(self, generated_tests: str, analysis: CodeAnalysis) -> str:
        ...
        if not any(line.startswith('#') or line.startswith('"""') for line in lines[:3]):
        ...
        for line in lines:
            ...
            if (line_stripped.startswith('from math_operations import') or
                line_stripped.startswith('import math_operations')):
                continue
            ...
            if 'assert' not in line and line_stripped.endswith('pass'):
                processed_lines.append(line.replace('pass', 'assert True  # TODO: Ajouter des assertions spécifiques'))
            else:
        ...
        if not self._validate_generated_tests(result):
            self.logger.warning("Les tests générés ne semblent pas valides")
            # Tentative de correction automatique
            corrected_result = self._fix_syntax_errors(result)
            if self._test_syntax_only(corrected_result):
                self.logger.info("Correction automatique des erreurs de syntaxe réussie")
    ...
    def _validate_generated_tests(self, test_code: str) -> bool:
        ...
        try:
            ...
            if corrected_code != test_code:
                self.logger.info("Tentative de correction automatique des erreurs de syntaxe...")
                try:
        ...
        while i < len(lines):
            ...
            if i + 1 < len(lines):
                ...
                needs_closing = (
                    ...
                    next_line.startswith('@pytest.mark')
                )
            else:
    ...
    def generate_test_suite_summary(self, source_files: list, analyses: list) -> str:
        ...
        total_methods = sum(sum(len(cls.methods) for cls in analysis.classes) for analysis in analyses)
        ...
        for source_file, analysis in zip(source_files, analyses):
            ...
            summary.append(f"- {rel_path}: {len(analysis.functions)} fonctions, {len(analysis.classes)} classes")
