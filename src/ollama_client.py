...
class OllamaClient:
        ...
        try:
            ...
            if self.config.model not in available_models:
                ...
                if not base_matches:
                    ...
                    self.logger.error(
                        f"Installez le modèle avec: ollama pull {self.config.model}"
                    )
                    return False
                else:
                    self.logger.warning(
                        f"Modèle exact '{self.config.model}' non trouvé."
                    )
                    self.logger.warning(
                        f"Modèles similaires disponibles: {', '.join(base_matches)}"
                    )
    ...
    def generate_response(self, prompt: str) -> Optional[str]:
        ...
        """
        self.logger.debug(
            f"Envoi du prompt à Ollama (taille: {len(prompt)} caractères)"
        )
        ...
        try:
            payload = {
                ...
                "options": {
                    ...
                    # Suppression des stop sequences qui causent des problèmes
                },
            }
            ...
            self.logger.debug(
                f"Réponse générée en {end_time - start_time:.2f}s (taille: {len(generated_text)} caractères)"
            )
            ...
            # Nettoyage minimal de la réponse pour préserver la syntaxe
            cleaned_text = self._clean_response_minimal(generated_text)
            ...
            if "404" in str(e):
                self.logger.error(
                    f"Le modèle '{self.config.model}' n'existe peut-être pas."
                )
                ...
                if models:
                    ...
                    self.logger.error(
                        "Aucun modèle trouvé. Installez un modèle avec: ollama pull <model_name>"
                    )
            return None
        except json.JSONDecodeError as e:
    ...
    def _clean_response(self, response: str) -> str:
        ...
        for line in lines:
            ...
            if line_stripped.startswith("```python") or line_stripped.startswith(
                "```py"
            ):
                in_code_block = True
            ...
            # Si aucun bloc de code n'a été détecté, on considère tout comme du code
                ...
                if not any(
                    ...
                    for word in ["voici", "voila", "ci-dessous", "following", "here"]
                ):
                    cleaned_lines.append(line)
        ...
        if not result and response.strip():
            self.logger.warning(
                "Le nettoyage a produit un résultat vide, retour de la réponse originale"
            )
    ...
    def _clean_response_minimal(self, response: str) -> str:
        """
        Nettoyage minimal de la réponse d'Ollama pour préserver la syntaxe

        Args:
            response: Réponse brute d'Ollama

        Returns:
            Code Python nettoyé avec syntaxe préservée
        """
        # D'abord essayer d'extraire le code des balises ```python
        ...
        # Sinon, supprimer seulement les lignes explicitement non-code
        lines = response.split("\n")
        cleaned_lines = []
        ...
        for line in lines:
            ...
            if (
                ...
                or line_stripped in ["", "code python:", "tests unitaires:"]
            ):
                continue
    ...
    def pull_model(self, model_name: str) -> bool:
        ...
        try:
            ...
            response = self.session.post(
                f"{self.base_url}/api/pull", json=payload, stream=True
            )
            response.raise_for_status()
            ...
            self.logger.error(
                f"Erreur lors du téléchargement du modèle {model_name}: {e}"
            )
            return False
